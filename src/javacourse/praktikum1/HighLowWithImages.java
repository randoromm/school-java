...
/**
 * This program is a simple card game. The user sees a card and tries to predict
 * whether the next card will be higher or lower. Aces are the lowest-valued
 * cards. If the user makes three correct predictions, the user wins. If not,
 * the user loses.
 *
 * This class defines a panel, but it also contains a main() routine that makes
 * it possible to run the program as a stand-alone application.
 *
 * This program depends on several additional source code files: Card.java,
 * Hand.java, and Deck.java. It also requires the image file, cards.png, which
 * contains the images of the playing cards. (The file cards.png is taken from
 * the Gnome Desktop.)
 */
public class HighLowWithImages extends JPanel {

	/**
	 * The main routine simply opens a window that shows a HighLowWithImages
	 * panel.
	 */
	 ...
	 * defined below). On the bottom is a panel that holds three buttons. The
	 * CardPanel listens for ActionEvents from the buttons and does all the real
	 * work of the program.
	 */
	...
	/**
	 * A nested class that displays the cards and does all the work of keeping
	...
	private class CardPanel extends JPanel implements ActionListener {
		 ...
		 * method. Note that the buttons are created and listening is set up in
		 * the constructor of the HighLowPanel class.
		 */
		...
		/**
		 * Called by actionPerformed() when user clicks "Higher" button. Check
		 * the user's prediction. Game ends if user guessed wrong or if the user
		 * has made three correct predictions.
		 */
		void doHigher() {
		...
		/**
		 * Called by actionPerformed() when user clicks "Lower" button. Check
		 * the user's prediction. Game ends if user guessed wrong or if the user
		 * has made three correct predictions.
		 */
		void doLower() {
		...
		/**
		 * Called by the constructor, and called by actionPerformed() if the use
		 * clicks the "New Game" button. Start a new game.
		 */
		void doNewGame() {
			if (gameInProgress) {
		...
		/**
		 * This method draws the message at the bottom of the panel, and it
		 * draws all of the dealt cards spread out across the canvas. If the
		 * game is in progress, an extra card is drawn face down representing
		 ...
		 *            If g is null, a NullPointerException will be thrown.
		 * @param card
